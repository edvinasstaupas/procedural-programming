...
int main() {
   ...
   printf("Programa apskaiciuoja sekoje pasikartojanciu skaiciu daznuma. Seka sudaro neneigiami skaiciai nuo 0 iki 1000 imtinai\n");
   // ask to enter 10 numbers
   printf("Iveskite 10 sekos nariu\n");

   while(counter < SIZE){
      // ask to enter the following number
      printf("Iveskite %d sekos nari: ", counter+1);
      if((scanf("%d", &num) == 1) && (getchar() == '\n')){
         if ((num >= 0) && (num <= FREQ_SIZE)){
            inputArray[counter++] = num;
            // report that number entered successfully
            printf("%d-as skaicius ivestas teisingai\n", counter);
         } else {
            // report that value exceeds bounds and has to belong to [0; FREQ-SIZE]
            printf("Ivestas neteisingas skaicius. Skaicius turi priklausyti [0; %d]\n", FREQ_SIZE);
         }
      } else {
         // report invalid input, ask to enter whole non-negative number again
         printf("Ivestas neteisingas skaicius. Skaicius turi buti neneigiamas sveikasis\n");
         while(getchar() != '\n')
            ;
      }
   }
